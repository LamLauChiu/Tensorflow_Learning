...
"""
    ...
    the details of which we will discuss much later.
"""
...
"""
    ...
    Without any annotations, TensorFlow automatically decides whether to use the GPU or CPU for an operation
    (and copies the tensor between CPU and GPU memory if necessary).
...
print(x.device.endswith('GPU:0'))                # False
...
"""
    ...
    This name encodes many details, such as an identifier of the network address of the host on which this program is executing and the device within that host.
    ...
    The term "placement" in TensorFlow refers to how individual operations are assigned (placed on) a device for execution.
    As mentioned above, when there is no explicit guidance provided,
    TensorFlow automatically decides which device to execute an operation,
    and copies Tensors to that device if needed. However,
...
def time_matmul(x):
    ...
    print("10 loops: {:0.2f}ms".format(1000 * result))      #   10 loops: 75.15ms
...
"""
    ...
    2. Iteration over a Dataset with eager execution enabled.
    ...
    If you're familiar with TensorFlow graphs, the API for constructing the Dataset object remains exactly the same
    when eager execution is enabled, but the process of iterating over elements of the dataset is slightly simpler.
    ...
    Dataset.from_tensor_slices or using objects that read from files like TextLineDataset or TFRecordDataset.
